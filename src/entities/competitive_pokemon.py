...
class CompetitivePokemon(Pokemon):
    ...
    def calculate_all_stats(self):
        self.base_stats['hp'] = self.calculate_hp(self.base_stats['hp'], self.ivs.get('hp'), self.evs.get('hp'), self.level)
        self.base_stats['atk'] = self.calculate_stat(self.base_stats['atk'], self.ivs.get('atk'), self.evs.get('atk'), self.level, natures[self.nature]['atk'])
        self.base_stats['def'] = self.calculate_stat(self.base_stats['def'], self.ivs.get('def'), self.evs.get('def'), self.level, natures[self.nature]['def'])
        self.base_stats['spatk'] = self.calculate_stat(self.base_stats['spatk'], self.ivs.get('spatk'), self.evs.get('spatk'), self.level, natures[self.nature]['spatk'])
        self.base_stats['spdef'] = self.calculate_stat(self.base_stats['spdef'], self.ivs.get('spdef'), self.evs.get('spdef'), self.level, natures[self.nature]['spdef'])
        self.base_stats['spd'] = self.calculate_stat(self.base_stats['spd'], self.ivs.get('spd'), self.evs.get('spd'), self.level, natures[self.nature]['spd'])
